...
import {
  CERTIFICATIONS_GUIDANCE,
  EXPERIENCE_PROJECT_GUIDANCE,
} from "./resume-extras.js";
import { BOLD_FORMATTING_GUIDANCE } from "./bold-formatting.js";
...
export const SUMMARY_OPENAI = `SUMMARY (paragraph — executive overview):
- Return "summary" as a single string (60–80 words, 3–4 sentences).
- Summarize overall expertise, architecture experience, technical breadth, business domains, cloud experience, mentoring, and engineering impact — not a replay of experience bullets.
- Use **bold** on 3–6 separate technology names in the summary — never an entire sentence. Do not paraphrase experience bullets.`;
...
- Use <b> on 3–6 separate technology names only — never an entire sentence.
...
- ${BOLD_FORMATTING_GUIDANCE}
- Never use **bold** in skills or certifications — plain text only.
...
- ${BOLD_FORMATTING_GUIDANCE}
- Use <b> only on individual tech terms in summary, per-role project lines, and experience bullets — not in skill or certification lists.
...
- Summary is a paragraph string, not bullets. Certifications section present (3–5 plain-text items). Every experience row has project.name + project.summary above details.
...
- Summary is a paragraph; certifications section below skills; each job has a project line above bullets.
- Summary does not duplicate or paraphrase experience bullets.
- ${HUMAN_READABILITY_CHECK}
...
/** Compact OpenAI-only prompt. Title/timeline/skill-bold cleanup runs server-side after generation. */
export const OPENAI_SYSTEM_PROMPT = `Expert technical recruiter. ATS senior engineer resume → valid JSON only (no markdown fences).

SPINE: exact company names, start_date, end_date; same row count; newest first.
TITLES: root title + experience[0].title = Senior + short role family. Older rows: realistic mid/junior progression. No stack/location in titles.
FORMAT: ${BOLD_FORMATTING_GUIDANCE} Never bold skills, certifications, titles, companies, or dates.
SUMMARY: one string, 60–80 words, 3–4 sentences — executive career overview (expertise, domains, architecture, cloud, mentoring). Not a replay of experience bullets.
SKILLS: 6–7 categories × 8–10 items (~65–80 total). Industry & Domain last. JD keywords first per category. Plain-text concrete technologies only.
${CERTIFICATIONS_GUIDANCE}
${EXPERIENCE_PROJECT_GUIDANCE}
EXPERIENCE BULLETS: 4–5 on latest role, 3–4 on older roles. 18–28 words; no periods; ≤3 tech per bullet. Vary architecture, ownership, collaboration, impact. ~60–70% with believable metrics. Distinct company domains; era-appropriate tech.
QUALITY: 90%+ JD keywords (skills + recent experience + projects). No duplicate bullets/metrics; no verb >2×; no buzzwords.`;

export const OPENAI_SYSTEM_PROMPT_RETRY = OPENAI_SYSTEM_PROMPT.replace(
  "60–80 words, 3–4 sentences",
  "50–70 words, 2–3 sentences"
)
  .replace("6–7 categories × 8–10 items (~65–80 total)", "5–6 categories × 7–9 items (~50–65 total)")
  .replace("4–5 on latest role, 3–4 on older roles", "3–4 on latest role, 3 on older roles")
  .replace("3–5 plain-text items", "3–4 plain-text items");

export const OPENAI_OUTPUT_SCHEMA = `{"title":"","summary":"","certifications":[],"skills":{"Category":[]},"experience":[{"title":"","company":"","location":"","start_date":"","end_date":"","project":{"name":"","summary":""},"details":[]}]}`;

export function buildOpenAiUserPrompt(candidateContext, jobDescription, yearsOverTen) {
  const yearsNote = yearsOverTen ? "\nSummary: say \"more than 10 years\" only, never an exact year count." : "";
