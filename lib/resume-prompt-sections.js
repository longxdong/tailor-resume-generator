...
FORMAT: **bold** in summary and experience bullets only — never in skills, titles, companies, or dates. Era-appropriate tech per role; simpler stacks in older/junior roles.
SUMMARY: 4 bullets — executive career overview (expertise, domains, architecture, cloud, mentoring). Do not paraphrase experience bullets.
SKILLS: 6–7 categories × 8–10 items (~65–80 total). Industry & Domain last. JD keywords first in each category. Plain-text concrete technologies (React, Kafka, PostgreSQL) — not vague concepts (Scalability, Modern Development). Primary ATS keyword coverage.
EXPERIENCE: 4–5 bullets on latest role, 3–4 on older roles. 18–28 words; no periods; ≤3 technologies per bullet. Vary emphasis: architecture, ownership, collaboration, business impact — not the same Verb+Tech+Metric pattern every time. ~60–70% bullets with believable metrics (15–40%, 2x–3x). Each employer keeps distinct domain identity; do not rewrite every company as the JD clone. Recent=architecture/mentoring; middle=features/APIs; older/junior=implementation/support.
QUALITY: 90%+ JD technical keywords across resume (skills + recent experience). No duplicate bullets, metrics, or verbs used more than twice. No buzzwords (synergy, passionate, cutting-edge, robust and scalable).`;
...
export const OPENAI_SYSTEM_PROMPT_RETRY = OPENAI_SYSTEM_PROMPT.replace(
  ...
  .replace("4–5 bullets on latest role, 3–4 on older roles", "3–4 bullets on latest role, 3 on older roles");

export const OPENAI_OUTPUT_SCHEMA = `{"title":"","summary":[],"skills":{"Category":[]},"experience":[{"title":"","company":"","location":"","start_date":"","end_date":"","details":[]}]}`;
